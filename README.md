...
- **标准模式能力完整**：文件编辑、Shell、文件与网页检索、Skills、计划、目标、子代理、工作流等全部保留，一次工具调用一次往返、中间可观察。
...
└── skills/
    ...
    ├── dsh-preset-authoring/     # DSH 预设创作速查手册（复制内置预设 + 丢 skill 即可，含踩坑清单）
...
四个技能通过 `skill-filesystem.customSkillDirs` 随预设挂载，模型会话里直接可见、可按需加载。
